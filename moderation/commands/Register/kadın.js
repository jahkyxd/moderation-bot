...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        var name = args[1]
        var age = args[2]
        if (!message.author.roles.cache.has(config.registration.staff) && !message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(embed.setDescription("Ne yazık ki komutu kullanan kişide yetki yok"));
        ...
        db.push(`isimler_${member.id}`, ` \`${name} ${config.registration.symbol} ${age}\` (<@&${config.registration.onewoman}>)`);
        db.add(`kadın_${message.author.id}`, 1)
        db.add(`toplam_${message.author.id}`, 1)
        db.push(`kke_${member.id}`, `${message.author} \`${moment(Date.now()).format("LLL")}\` (<@&${config.registration.onewoman}>)`)
        await message.guild.members.cache.get(member.id).roles.add(config.registration.woman);
